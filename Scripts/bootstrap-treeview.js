(function ($) {
    ...
    function GTreeTable(element, options) {
        ...
        this.template = this.options.template !== undefined ? this.options.template : 
           ...
           '<td>' +
		   '<span tabindex="0" class="node-container">${draggableIcon}${indent}${ecIcon}${typeIcon}${name}</span>' +
           //'<span>${draggableIcon}${indent}${ecIcon}${selectedIcon}${typeIcon}${name}</span>' +
           //'<span class="hide ' + this.options.classes.action + '">${input}${saveButton} ${cancelButton}</span>' + TONG: commented out, not using. 
           //'<div class="btn-group pull-right ' + this.options.classes.buttons + '">${actionsButton}${actionsList}</div>' +	TONG: commented out, not using. 		   
           '</td>' +
           '</tr>' +
           '</table>';            
	
        if (this.options.linkDefaultActionToDblClick) {
        	this.template = this.options.template !== undefined ? this.options.template :
           '<table class="table gtreetable">' +
           '<tr class="' + this.options.classes.node + ' ' + this.options.classes.collapsed + '">' +
           '<td>' +
		   '<span tabindex="0" class="node-container">${draggableIcon}${indent}${ecIcon}${typeIcon}${name}${defaultAction}</span>' +           
           '</td>' +
           '</tr>' +
           '</table>';
        }
      ...
      this.templateParts = this.options.templateParts !== undefined ? this.options.templateParts :
            {
                ...
                typeIcon: '<span class="' + this.options.classes.typeIcon + '">&nbsp &nbsp &nbsp</span>',
                ...
                actionsList: '',
                defaultAction: '<span class="glyphicon glyphicon-plus pull-right defaultAction" tabindex="-1"> </span>'
    ...
    GTreeTable.prototype = {
        ...
        getSourceNodes: function (nodeId, force) {
            ...
            var sourceOptions = this.options.source(nodeId, this);
			...
			/* Tong: if we have a local json node tree , build the tree from the local data rather than going to the server*/
            if (this.options.nodeJSON) {
            	var mock = $.Deferred(), options = this.options;
            	var data = options.nodeJSON;
            	if (sourceOptions.data && nodeId != 0) {
        ...
        init: function () {
            ...
            this.getSourceNodes(0).done(function (result) {
                ...
                if (result && result.Results) {
                    lookupLength = result.Results.length;
                    that.noOfNodes = lookupLength;
                }

                $.event.trigger({ type: "treeDataLoaded", message: lookupLength, time: new Date() });
                ...
                $.event.trigger({ type: "treeRenderingCompleted", message: "Rendering Completed", time: new Date() });
            });
        },
    ...
    function GTreeTableNode(data, gtreetable) {
        ...
        this.type = data.type;
        this.id = data.id;
        this.number = data.Idnumber;

        
    	//Tong: Copy other custom data being passed in as well
        for (var i in data) {
    ...
    GTreeTableNode.prototype = {
        ...
        getNumber: function () {
            return this.isEditable() ? this.$input.val() : this.number;
        },
        ...
        init: function () {
            ...
            this.$number = this.$node.find('.' + this.manager.options.classes.number);
            ...
            this.$defaultAction = this.$node.find('.defaultAction' );
        ...
        render: function () {
            ...
            this.$number.html(this.number);
        	if (this.renderNameInPreTag) {
                this.$name.html('<pre>' + this.name + '</pre>');
                this.$number.html('<pre>' + this.number + '</pre>');
        	}

            this.$name.attr("title", this.name);
            this.$number.attr("title", this.number);
        ...
        attachEvents: function () {
			...
			//Tong: add in dblclick event handler
            this.$name.dblclick(function(e){
            	if(that.manager.options.dblclick){
            		that.manager.options.dblclick(that);
            	}
            });

        	//Tong: add in attach defaultAction event handler
            this.$defaultAction.click(function (e) {
            	if (that.manager.options.dblclick) {
            		that.manager.options.dblclick(that);
            	}
            });
            ...
            if (isNaN(selectLimit) === false && (selectLimit > 0 || selectLimit === -1)) {            	
                ...
                this.$node.keyup(function (data, e) {
                    switch (data.keyCode) {
                        case 13:
                            {
                                clickAction();
   ...
   function GTreeTableCache(manager) {
    ...
    function Plugin(option, _relatedTarget) {
       ...
       var $this = $(this)
        if ($this.data('bs.gtreetable') && $this.data('bs.gtreetable').attachedEventCount != undefined && !hasUpdated) {
            $this.data('bs.gtreetable').attachedEventCount = $this.data('bs.gtreetable').attachedEventCount + 1;

            if ($this.data('bs.gtreetable').attachedEventCount == $this.data('bs.gtreetable').noOfNodes) {
                $.event.trigger({ type: "eventAttachmentCompleted", message: "Event Attachment Completed", time: new Date() });
                hasUpdated = true;
            }
        }
